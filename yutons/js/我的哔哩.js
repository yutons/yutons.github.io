...
var rule = {
    ...
    class_name:json.classes.map(classObj => classObj.type_name).join('&'),
    class_url:json.classes.map(classObj => classObj.type_id).join('&'),
    // class_parse方法暂不可用
    /*class_parse:`js:
    let html=request('http://192.168.31.170:5705/js/%E5%B0%8F%E5%AD%A6%E6%95%99%E8%82%B2.json');
    log('===========================')
    input=JSON.parse(html);
    log(input)
    `,*/
    filterable: 1,
