 ...
 */
let bili_env = {
    "bili_cookie": "buvid3=666",
    "vmid": "675984140"
};
var rule = {
    title:'哔哩影视',
    host:'https://api.bilibili.com',
    url:'/fyclass-fypage&vmid='+bili_env.vmid,
    ...
    一级:`js: let d = [];
			...
			function get_result(url) {
				...
				let jo = JSON.parse(html.replace(/\\n/g,'').replace(/\\r/g,'').replace(/\\"keyword\\"/g,'"keyword"'));
				if (jo["code"] === 0) {
			...
			function get_rank(tid, pg) {
				return get_result("https://api.bilibili.com/pgc/web/rank/list?season_type=" + tid + "&pagesize=20&page=" + pg +
					"&day=3")
			}

			function get_rank2(tid, pg) {
				return get_result("https://api.bilibili.com/pgc/season/rank/web/list?season_type=" + tid + "&pagesize=20&page=" +
					pg + "&day=3")
			}

			function get_zhui(pg, mode) {
				let url = "https://api.bilibili.com/x/space/bangumi/follow/list?type=" + mode + "&follow_status=0&pn=" + pg +
					"&ps=10&vmid=" + vmid;
				return get_result(url)
			}

			function get_all(tid, pg, order, season_status) {
				let url = "https://api.bilibili.com/pgc/season/index/result?order=" + order +
					"&pagesize=20&type=1&season_type=" + tid + "&page=" + pg + "&season_status=" + season_status;
				return get_result(url)
			}
